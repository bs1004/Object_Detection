...
def load_cocojson(filename):
    """ Gets label data from a coco label file
    :param (str) filename: file path to a coco label file
    :return: (numpy.ndarray, numpy.ndarray ,numpy.ndarray) coords, chips, and classes corresponding to
            the coordinates, image names, and class codes for each ground truth.
    """
    ...
    # 빈공간 만들기
    obj_coords = np.zeros((len(data['annotations']), 8))# bounding box 좌표
    image_ids = np.zeros((len(data['images'])), dtype='object') # 이미지 수
    class_indices = np.zeros((len(data['features'])), dtype=int) # 클래스(숫자) 
    class_names = np.zeros((len(data['features'])), dtype='object') # 클래스명(영어)
    
    #인덱스를 통해서 부여하기
    for idx in range(len(data['features'])): 
        properties = data['features'][idx]['properties']
        image_ids[idx] = properties['image_id'] # 이미지명
        obj_coords[idx] = np.array([float(num) for num in properties['bounds_imcoords'].split(",")]) #바운딩 박스 스플릿
        class_indices[idx] = properties['type_id']
        class_names[idx] = properties['type_name']
...
def create_tfrecords(src_dir, dst_path, patch_size=1024, patch_overlay=384, object_fraction_thresh=0.7,
    ...
    print('N of Patches:', len(patches))
